        ...
        <div className="min-h-screen bg-transparent pt-32 px-4 md:px-10 pb-12">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8 reveal">
                    <div>
                        <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-black tracking-tighter leading-none mb-4">
                    ...
                    <button onClick={openPostModal} className="tactile-btn w-full md:w-auto px-10 py-5 bg-black text-white rounded-2xl font-black text-xl flex items-center justify-center shadow-md hover:bg-forest transition-all">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
                    ...
                    <div className="lg:col-span-2 space-y-10 reveal">
                        <h2 className="text-xl md:text-2xl font-black text-black tracking-widest uppercase flex items-center">
                            <Briefcase className="w-6 h-6 mr-3 text-coral" /> Active Bounties
                        </h2>
                        
                        {myProblems.length === 0 ? (
                            <div className="tactile-card p-12 text-center bg-white rounded-[2rem] border-dashed">
                                <p className="text-gray-400 font-black uppercase tracking-widest">No active protocols detected.</p>
                            ...
                            <div key={p.id} className={`tactile-card p-6 md:p-10 rounded-[2.5rem] bg-white transition-all duration-300 ${idx % 2 === 0 ? 'rotate-[-0.5deg]' : 'rotate-[0.5deg]'} hover:rotate-0`}>
                                <div className="sticker-tape opacity-10"></div>
                                <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                                    <h3 className="text-2xl md:text-3xl font-black text-black tracking-tighter hover:text-coral transition-colors cursor-pointer" onClick={() => openProblemDetails(p)}>{p.title}</h3>
                                    <div className="flex items-center gap-3 w-full sm:w-auto">
                                        <button onClick={() => openEditModal(p)} className="flex-1 sm:flex-none p-3 bg-paper border-2 border-black rounded-xl hover:bg-citrus transition-all flex items-center justify-center"><Edit2 className="w-5 h-5" /></button>
                                        {p.status === 'OPEN' && <button onClick={() => handleCloseProblem(p.id)} className="flex-1 sm:flex-none p-3 bg-paper border-2 border-black rounded-xl hover:bg-coral hover:text-white transition-all flex items-center justify-center"><Power className="w-5 h-5"/></button>}
                                        <span className={`px-4 py-1.5 rounded-full border-2 border-black font-black text-[10px] uppercase tracking-[0.2em] whitespace-nowrap ${p.status === 'OPEN' ? 'bg-citrus' : 'bg-gray-100 text-gray-400'}`}>{p.status}</span>
                                    </div>
                                </div>
                                ...
                                <div className="pt-8 border-t-2 border-black/5">
                                    ...
                                    <div className="space-y-4">
                                        {p.solutions?.map(s => (
                                            <div key={s.id} className={`tactile-card flex flex-col md:flex-row justify-between items-center p-6 rounded-2xl gap-4 ${s.isAccepted ? 'bg-citrus/10 border-forest' : 'bg-gray-50 border-black/5 shadow-none'}`}>
                                                ...
                                                <div className="flex gap-4 w-full md:w-auto">
                                                    {s.attachmentUrl && <a href={s.attachmentUrl} target="_blank" className="flex-1 md:flex-none p-3 bg-white border-2 border-black rounded-xl hover:scale-110 transition-transform flex items-center justify-center"><Download className="w-5 h-5 text-black"/></a>}
                                                    {p.status === 'OPEN' && !s.isAccepted && <button onClick={() => setAcceptModalOpen(s)} className="flex-[2] md:flex-none tactile-btn bg-black text-white px-6 py-2 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-forest">Audit & Accept</button>}
                                                    {s.isAccepted && <span className="flex-1 md:flex-none px-4 py-2 bg-forest text-citrus rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]"><CheckCircle2 className="w-4 h-4"/> Accepted</span>}
                                                </div>
                                            </div>
                                        ))}
                    ...
                    <div className="space-y-12 reveal">
                        <div className="tactile-card bg-black text-white p-8 md:p-10 rounded-[3rem] overflow-hidden">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-citrus/10 rounded-full -mr-16 -mt-16"></div>
                            <h2 className="text-xl md:text-2xl font-black tracking-widest uppercase mb-8 flex items-center gap-3">
                        ...
                        <div className="tactile-card bg-citrus p-8 md:p-10 rounded-[3rem]">
                           <h3 className="text-xl font-black mb-4">Pro Tip: 🪄</h3>
                           <p className="font-bold text-black/70 leading-relaxed text-sm md:text-base">Use the AI Polish tool to make your descriptions technical and clear. Better briefs get better results! 😁</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <form onSubmit={handleFormSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center mb-1">
                           ...
                           <button type="button" onClick={handleAIRefine} className="px-3 py-1 bg-black text-citrus rounded-lg font-black text-[9px] uppercase tracking-widest flex items-center hover:bg-forest transition-all">
                                {isRefining ? <Loader2 className="w-3.5 h-3.5 mr-2 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 mr-2" />} Gemini Refine
                           </button>
                        </div>
                        <textarea required className="w-full border-2 border-black p-5 rounded-2xl h-48 font-bold bg-paper shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] focus:translate-x-1 focus:translate-y-1 focus:shadow-none transition-all outline-none text-sm" placeholder="Provide deep technical context..." value={desc} onChange={e => setDesc(e.target.value)} />
                    </div>
            ...
            <Modal isOpen={!!acceptModalOpen && !showPaymentGateway} onClose={() => setAcceptModalOpen(null)} title="Approve Execution">
                <div className="space-y-8">
                    ...
                    <div className="bg-forest/5 p-6 rounded-2xl border-2 border-forest/10 flex items-start gap-4">
                        <Info className="w-6 h-6 text-forest mt-1 shrink-0" />
                        <p className="text-xs md:text-sm font-bold text-forest leading-relaxed">FixMyProblem Protocol handles all tax compliance and platform overhead. The payout is final and instant upon confirmation. 😁</p>
                    </div>
                    ...
                    <textarea className="w-full border-2 border-black p-4 rounded-2xl h-32 font-bold bg-paper focus:ring-0 outline-none text-sm" placeholder="Encourage your solver..." value={feedback} onChange={e => setFeedback(e.target.value)} />
                    
                    <button onClick={handleAcceptClick} className="tactile-btn w-full bg-coral text-white py-5 rounded-2xl font-black text-lg md:text-xl uppercase tracking-widest flex items-center justify-center gap-4 shadow-xl">
