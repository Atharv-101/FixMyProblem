        ...
        <div className="min-h-screen bg-transparent pt-32 px-4 pb-12">
           <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-12 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
                    <div className="space-y-4 reveal">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-black text-white rounded-xl text-[10px] font-black uppercase tracking-widest border border-black shadow-[4px_4px_0px_0px_rgba(253,224,71,1)]">
                        ...
                        >
                            <div className="w-14 h-14 rounded-2xl border-2 border-black bg-citrus overflow-hidden flex items-center justify-center font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                            ...
                            <div>
                                <span className="text-2xl font-black hover:text-coral transition-colors">{currentUserData?.name}</span>
                                <div className="text-[10px] font-black uppercase tracking-widest text-gray-400">IIT Kharagpur Grid</div>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-6 w-full lg:w-auto reveal">
                       <div className="tactile-card bg-forest text-white p-6 rounded-3xl flex flex-col justify-between">
                          <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-4">Net Extraction</p>
                          <div className="flex items-center text-3xl font-black">
                             <IndianRupee className="w-5 h-5 mr-1" />
                             {totalEarned.toLocaleString('en-IN')}
                          </div>
                       </div>
                       <div className="tactile-card bg-citrus text-black p-6 rounded-3xl flex flex-col justify-between border-black">
                          <p className="text-[10px] font-black uppercase tracking-widest opacity-60 mb-4">Reputation Score</p>
                          <div className="flex items-center text-3xl font-black">
                ...
                <div className="mb-12 relative group reveal">
                    ...
                    <input 
                       type="text" 
                       placeholder="Scan roadblocks by tech, bounty or ID..." 
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {filteredProblems.map((p, idx) => (
                        <div 
                            ...
                            className={`tactile-card p-8 rounded-[2.5rem] cursor-pointer group flex flex-col h-full reveal transition-all duration-300 ${idx % 2 === 0 ? 'rotate-[-0.5deg]' : 'rotate-[0.5deg]'} hover:rotate-0`}
                        >
                            <div className="sticker-tape opacity-10 group-hover:opacity-100 transition-opacity"></div>
                            
                            <div className="flex justify-between items-start mb-6">
                                <div className="px-3 py-1 bg-gray-50 border-2 border-black rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                                    <Target className="w-3 h-3 text-coral" />
                                    {p.companyName}
                                </div>
                                <div className="text-right">
                                    <div className="flex items-center font-black text-forest text-2xl">
                            ...
                            <h3 className="text-2xl font-black text-black mb-4 line-clamp-2 leading-tight group-hover:text-coral transition-colors underline decoration-black/5">{p.title}</h3>
                            ...
                            <div className="flex flex-wrap gap-2 mb-8">
                                {p.tags.slice(0, 3).map(tag => (
                                    <span key={tag} className="text-[9px] font-black uppercase tracking-[0.2em] bg-citrus/20 border border-black/5 px-2 py-1 rounded">
                                        {tag}
                                    </span>
                                ))}
                            </div>
                            ...
                            <button 
                                ...
                                className={`tactile-btn w-full py-5 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center transition-all ${p.status === 'OPEN' ? 'bg-black text-white hover:bg-coral' : 'bg-gray-100 text-gray-400 cursor-not-allowed border-black/10 shadow-none'}`}
                            >
                                {p.status === 'OPEN' ? (
                                    <>
                ...
                {filteredProblems.length === 0 && (
                    <div className="text-center py-40 tactile-card border-dashed bg-white/50 rounded-[3rem]">
                        <Terminal className="w-20 h-20 text-gray-200 mx-auto mb-8 animate-pulse" />
                        <p className="text-gray-400 text-2xl font-black uppercase tracking-widest">Awaiting Grid Inputs...</p>
                        <button onClick={() => setSearchQuery('')} className="mt-6 text-coral font-black underline decoration-citrus decoration-4 underline-offset-8 text-lg">Reset Filter</button>
                    </div>
                )}
           </div>

           {/* Modals are already quite tactile but let's ensure they are using App components */}
           <ProblemDetailModal
           ...
           <Modal isOpen={!!selectedProblemIdForSubmission} onClose={() => setSelectedProblemIdForSubmission(null)} title="Deploy Solution Protocol">
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div>
                        <label className="text-xs font-black uppercase tracking-widest text-gray-400 block mb-3">Project Link</label>
                        <div className="p-4 bg-gray-100 border-2 border-black rounded-xl font-black text-forest">
                            {activeProblemForSubmission?.title}
                        </div>
                    </div>

                    <div>
                        <label className="text-xs font-black uppercase tracking-widest text-gray-400 block mb-3">Execution Payload (Code/Logic)</label>
                        <textarea 
                            required 
                            className="w-full border-2 border-black rounded-2xl p-6 h-64 font-mono text-sm focus:ring-0 outline-none transition-all shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] focus:translate-x-1 focus:translate-y-1 focus:shadow-none bg-paper" 
